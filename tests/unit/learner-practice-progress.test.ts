...
const item = (id: string, correctKey: string): ExamItem =>
  ({ id, number: 1, stem: "s", answerFormat: "MC_SINGLE_3",
     options: [{ key: "a", text: "A" }, { key: "b", text: "B" }], correctKey,
     stimulusSlug: null } as ExamItem);
const part: ExamPart = { id: "p1", teilNumber: 1, label: "Teil 1", partKind: "X",
  durationMinutes: 10, instructions: "", items: [item("i1", "a"), item("i2", "b")] } as ExamPart;
...
describe("practiceProgress storage + chip/lock models", () => {
  ...
  it("save → load → list round-trips a record; Dexie row is snake_case", async () => {
    await savePracticeProgress({
      userId: "u1", board: "telc", level: "B1", moduleCode: "LESEN", quizSlug: "s1",
      answers: { i1: { key: "a", correct: true } }, correctCount: 1, totalCount: 10,
      completedAt: null, updatedAt: 111,
    });
    const loaded = await loadPracticeProgress({ userId: "u1", board: "telc", level: "B1", moduleCode: "LESEN", quizSlug: "s1" });
  ...
  it("deriveChip: null→todo, locked 0→todo even with a row, partial→inProgress, completed→done", () => {
    ...
    expect(deriveChip({ lockedCount: 0, totalCount: 10, completedAt: null })).toEqual({ state: "todo" });
    expect(deriveChip({ lockedCount: 3, totalCount: 10, completedAt: null })).toEqual({ state: "inProgress", locked: 3, total: 10 });
    expect(deriveChip({ lockedCount: 10, totalCount: 10, completedAt: 5 })).toEqual({ state: "done" });
  });
