...
describe("acknowledgeOnServer", () => {
  ...
  it("POSTs { submission_id, module } and resolves on 200", async () => {
    ...
    await expect(acknowledgeOnServer({ submissionId: "s1", module: "sprechen" })).resolves.toBeUndefined();
  ...
  it("treats 404 as idempotent success, rethrows other errors", async () => {
    ...
    await expect(acknowledgeOnServer({ submissionId: "s1", module: "schreiben" })).resolves.toBeUndefined();
    invokeFn.mockRejectedValueOnce(new ApiError(500, "db_error"));
    await expect(acknowledgeOnServer({ submissionId: "s1", module: "schreiben" })).rejects.toThrow("db_error");
  });
});
